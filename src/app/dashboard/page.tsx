...
import { signOut, useSession } from "next-auth/react"; // 1. Added NextAuth hooks

export default function DashboardPage() {
  // 2. Use real session data instead of hardcoded "Arya"
  const { data: session } = useSession();
  const firstName = session?.user?.name?.split(" ")[0] ?? "User";
  const email = session?.user?.email ?? "user@provus.ai";
  ...
  // 3. Proper NextAuth Logout
  const handleLogout = async () => {
    console.log("Initiating Logout...");
    await signOut({ 
      callbackUrl: "/login", // Redirects to login page after clearing session
      redirect: true 
    });
  };
  ...
  return (
    <div className="min-h-screen bg-[#060808] text-slate-100 font-sans relative overflow-x-hidden selection:bg-lime-500/30">
      ...
      {/* Navbar receives the proper signOut function */}
      ...
      <main className="relative z-10 max-w-[1200px] mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="flex flex-col gap-6">
            <GlassPanel onClick={handleJoinRoom} className="flex-1 group hover:border-blue-500/30 hover:bg-white/[0.04] cursor-pointer">
            ...
            <GlassPanel onClick={handleHistory} className="flex-1 group hover:border-pink-500/30 hover:bg-white/[0.04] cursor-pointer">
              <div className="p-6 flex flex-col justify-between h-full">
